...
const apiKey = "AIzaSyBbp3HuD3mfzyi-QWf3RSd-EQoy8puynWk"; 
...
const DashboardSection = ({ networkSpeed, isDarkMode }) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      const newLog = {
        id: Date.now(),
        source: ['Security', 'System', 'Application', 'Service'][Math.floor(Math.random() * 4)],
        eventID: 4624 + Math.floor(Math.random() * 50),
        message: 'Audit Success: User authentication verified via Kerberos protocol.',
  ...
  return (
    <div className="flex flex-col gap-6 pb-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Events" value="1,204,592" change="+12%" icon={<Database />} color="blue" isDarkMode={isDarkMode} />
        <StatCard title="Critical Threats" value="3" change="-2" icon={<ShieldAlert />} color="red" isDarkMode={isDarkMode} />
        <StatCard title="Warning Alerts" value="18" change="+5" icon={<AlertTriangle />} color="orange" isDarkMode={isDarkMode} />
        <StatCard title="Successful Logs" value="45,200" change="Normal" icon={<CheckCircle />} color="emerald" isDarkMode={isDarkMode} />
      </div>
